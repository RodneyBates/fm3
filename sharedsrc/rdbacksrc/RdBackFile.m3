...
; REVEAL T
  = BRANDED "RdBackFile.T" REF RECORD 
    ...
    ; RbCoordL : LONGINT := 0L    (* These are relative to the byte no ... *)
...
(* TODO: The above two fields do not really belong in this abstraction, but
         it's a lot of work to put them into a subtype.
*)
...
(* The file is handled in fixed-sized blocks of size BlockSize.  RbBuffer is
   ...
   the block to the right, after maybe flushing its old contents to disk.
...
; PROCEDURE ReadHeader ( RbFile : T ; MsgTag : TEXT )
  ...
  ; BEGIN (*ReadHeader*)
    ...
    ; RbFile ^ . RbCoordL := FM3UnsafeUtils . Bytes8ToLongInt ( LBytes8 ) 
    ; ReadDisk
        ( RbFile
...
; PROCEDURE WriteHeader ( RbFile : T ; MsgTag : TEXT )
  ...
  ; BEGIN (*WriteHeader*)
    ...
    ; LBytes8 := FM3UnsafeUtils . LongIntToBytes8 ( RbFile ^ . RbCoordL ) 
    ; WriteDisk
        ( RbFile
        , PrefixSizeL
        , BYTESIZE ( Bytes8 )
...
; PROCEDURE InnerOpen 
  ...
  ; BEGIN (*InnerOpen*) 
    ...
    ; TRY
      ...
      => Raise
           ( MsgLabel , ", unable to " , MsgLabel , FileName , "." , Code := Code )
      END (*EXCEPT*)
...
; PROCEDURE Create ( FileName : TEXT ) : T
  ...
  ; BEGIN
      LRbFile 
        := InnerOpen ( FileName , "Create" , Truncate := TRUE )
     ; IF LRbFile ^ . RbDiskLengthL > 0L
       THEN 
         TRY LRbFile ^ . RbFileHandle . close ( ) 
         EXCEPT OSError . E 
         => 
         END (*EXCEPT*)
       ; Raise ( "To be created file " , FileName , " is not empty." )
       END (*IF*)
    ; LRbFile ^ . RbBlockNo := 0 
    ; LRbFile ^ . RbBlockCt := 0 
    ; LRbFile ^ . RbCoordL := 0L 
...
; PROCEDURE Open ( FileName : TEXT ) : T
  ...
  ; BEGIN
      LRbFile 
        := InnerOpen ( FileName , "Open" , Truncate := FALSE  )
    ; ReadHeader ( LRbFile , "Open" ) 
    ; IF LRbFile ^ . RbCoordL <= 0L
      THEN 
        LRbFile ^ . RbBlockNo := 0 
      ; LRbFile ^ . RbBlockCt := 0 
      ; LRbFile ^ . RbBlockNextIn := 0
      ...
      ; LRbFile ^ . RbBlockNo
          := VAL ( ( LRbFile ^ . RbCoordL - 1L ) DIV BlockSizeL
                 , INTEGER
                 ) 
      ; LRbFile ^ . RbBlockNextIn
          := VAL
               ( ( ( LRbFile ^ . RbCoordL -1L ) MOD BlockSizeL ) + 1L
               , INTEGER
               )
      ; LRbFile ^ . RbBlockCt
...
(*EXPORTED.*)
; PROCEDURE Seek ( RbFile : T ; CoordL : LONGINT ) 
  ...
  ; BEGIN (*Seek*)
      IF CoordL < 0L THEN LCoordL := 0L
      ELSIF CoordL > RbFile ^ . RbMaxLengthL
      THEN LCoordL := RbFile ^ . RbMaxLengthL
      ELSE LCoordL := CoordL
      END (*IF*) 
    ; IF LCoordL = RbFile ^ . RbCoordL 
      THEN RETURN LCoordL
      END (*IF*) 
    ; LNewBlockNo := VAL ( LCoordL DIV BlockSizeL , INTEGER ) 
    ; LNewNextIn := VAL ( ( ( LCoordL -1L ) MOD BlockSizeL ) + 1L , INTEGER )
...
; PROCEDURE Put ( RbFile : T ; Value : ByteTyp )
  ...
  = BEGIN
    ...
    ; IF RbFile ^ . RbBlockNextIn >= BlockSize
      ...
      ; IF RbFile ^ . RbMaxLengthL > RbFile ^ . RbCoordL + 1L 
    ...
    ; INC ( RbFile ^ . RbCoordL )
    ; RbFile ^ . RbMaxLengthL := MAX ( RbFile ^ . RbMaxLengthL , RbFile ^ . RbCoordL )
    END Put 
...
; PROCEDURE GetBwd
  ...
  ; BEGIN
    ...
    ; IF RbFile ^ . RbCoordL <= (* < shouldn't happen. *) 0L
      THEN RAISE BOF
      END (*IF*)
