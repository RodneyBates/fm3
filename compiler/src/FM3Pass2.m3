...
; PROCEDURE Pass2Tokens ( LMPass1Coord : LONGINT )
  ...
  ; BEGIN (* Pass2Tokens *) 
    ...
    END Pass2Tokens
(*
; PROCEDURE AssertExprStack1 ( VAR (*OUT*) Top : FM3Exprs . ExprRefTyp )
...
; PROCEDURE AssertExprStackGE1 ( VAR (*OUT*) Top : FM3Exprs . ExprRefTyp )
...
; PROCEDURE AssertExprStack2 ( VAR (*OUT*) Top , Second : FM3Exprs . ExprRefTyp )
...
; PROCEDURE AssertExprStackGE2
    ( VAR (*OUT*) Top , Second : FM3Exprs . ExprRefTyp )
...
; PROCEDURE PushExprIgnore
    ( READONLY Position : tPosition ; Kind := Ekt . EkNull )
  (* Push an expression node with only a position.
     Could be legally absent or unusable on account of some static error.
  *) 

  = VAR LNewExprRef : FM3Exprs . ExprRefTyp 

  ; BEGIN 
      LNewExprRef := FM3Exprs . NewExprRef ( ) 
    ; LNewExprRef ^ . ExpKind := Kind  
    ; LNewExprRef ^ . ExpPosition := Position
...
; PROCEDURE AdoptOutExprRef ( OrphanExprRef : FM3Exprs . ExprRefTyp )
  (* PRE: NOT Skipping. *)
  (* A value- or type-expression, maybe part of a decl. *) 

  = VAR LScopeRef : FM3Scopes . ScopeRefTyp
  ; VAR LParentExprRef : FM3Exprs . ExprRefTyp 
  
  ; BEGIN (*AdoptOutExprRef*) 
      IF FM3Exprs . ExprStackTopObj = NIL
      THEN (* Top node of an expr tree. *)
        IF AreInsideADecl ( ) (* Right now, always TRUE. *) 
        THEN (* Temporarily store the expr in the top scope, from where it will
                eventually be copied into the type or value of one or more
                decls. They will be created when the ident list is available.
             *) 
          LScopeRef := FM3Scopes . ScopeDeclStackTopRef
        ; WITH WCurDef
             = LScopeRef ^ . ScpCurDefExprs [ LScopeRef ^ . ScpCurDefIsValue ]
          DO IF WCurDef # NIL
            THEN  <* ASSERT FALSE , "duplicate def expression." *>
            END (*IF*) 
          ; WCurDef := OrphanExprRef
          END (*WITH*) 
        ELSE (* Top level, but not in an FM3Decls.DeclTyp. *)
(* CHECK: Is there any case where this needs to happen? *) 
          PutBwdP2
            ( P2RdBack , VAL ( OrphanExprRef ^ . ExpPosition . Column , LONGINT ) )
        ; PutBwdP2
            ( P2RdBack , VAL ( OrphanExprRef ^ . ExpPosition . Line , LONGINT ) ) 
        ; PutBwdP2 ( P2RdBack , VAL ( OrphanExprRef ^ . ExpSelfExprNo , LONGINT ) )
        ; PutBwdP2 ( P2RdBack , VAL ( Itk . ItkRefTopExprNo , LONGINT ) )
        END (*IF*) 
      ELSE (* A proper subexpression. *)
        (* Its incomplete parent-to-be is on top of expr stack. *) 
        (* Inherit some things from parent expr node. *)
        OrphanExprRef ^ . ExpIsLegalRecursive
          := OrphanExprRef ^ . ExpIsLegalRecursive
             OR FM3Exprs . ExprStackTopObj ^ . ExpIsLegalRecursive
        (* ^Oh how tricky. OrphanExprRef could be a builtin that only one global
            instance of exists, not to be changed.  But if so,
            ExpIsLegalRecursive will already be TRUE, so no change.
        *) 
      ; FM3Exprs . PushExprStack ( OrphanExprRef )
        (* From where it will become a child of the parent, at the right place,
           before we reach the Rt end of the parent.
        *) 
      END (*IF*) 
    END AdoptOutExprRef 
...
; PROCEDURE DefExprLt ( ExprRef : FM3Exprs . ExprRefTyp )
  (* PRE NOT Skipping. *)
...
; PROCEDURE UnaryOp ( Opcode : FM3Exprs . OpcodeTyp )
  ...
  = VAR LUnOpExpr : FM3Exprs . ExprRefTyp

  ; BEGIN
      LUnOpExpr := FM3Exprs . ExprStackTopObj 
    ; IF
FALSE AND
...
; PROCEDURE BinaryOp ( Opcode : FM3Exprs . OpcodeTyp )
  ...
  = VAR LBinOpExpr : FM3Exprs . ExprRefTyp
  ; VAR LOpnd1 , LOpnd2 : FM3Exprs . ExprRefTyp 

  ; BEGIN
      LBinOpExpr := FM3Exprs . ExprStackTopObj 
    ...
    ; IF LBinOpExpr ^ . ExpIsUsable
      ...
      ; LBinOpExpr ^ . ExpIsConst
          := LOpnd1 ^ . ExpIsConst
               ...
               AND IntSets . IsElement
                     ( LBinOpExpr ^ . ExpOpcode
                     , FM3Std . WordLongQualifierSet
                     )
...
; PROCEDURE DeclValue ( DeclKindTag : TEXT ; MustBeConst : BOOLEAN ) 
  ...
  = VAR  LValueExpr : FM3Exprs . ExprRefTyp 
...
*)
      ...
      THEN
        FM3Messages . ErrorArr 
          ( ARRAY OF REFANY
              { DeclKindTag , " is not a value expression." } 
...
; PROCEDURE AssertTosDeclScopeNo
  ...
  = BEGIN (*AssertTosDeclScopeNo*)
    ...
    ; IF FM3Scopes . ScopeDeclStackTopRef ^ . ScpSelfScopeNo # ExpectedScopeNo
      THEN <* ASSERT FALSE , "Scope no mismatch for " & Label *>
      END (*IF*) 
...
; PROCEDURE HandleTok ( READONLY TokResult : TokResultTyp ) 
  ...
  ; VAR LExprRef , LNewExprRef , LValueExpr , LPrefixExpr , LListElem
          : FM3Exprs . ExprRefTyp
...
TRUE OR
              AreInsideADecl ( )
          THEN 
            WITH LExpr = FM3Exprs . NewExprRef ( )  
            DO 
            ...
            ; FM3Utils . ContribToHashL ( LExpr ^ . ExpHash , LLongInt ) 
            ; FM3Utils . ContribToHashI ( LExpr ^ . ExpHash , LOpcode ) 
            ; LExpr ^ . ExpRepExprNo := FM3Exprs . ExprNoNull 
...
(* DECIDE: Do we want just one token code with a LoTyp operand here? *) 
  ...
  ; PROCEDURE HtExprRt
      ( NewExprRef : FM3Exprs . ExprRefTyp ; Mergeable : BOOLEAN )

    = BEGIN 
(* TODO: Maybe inline this. *)
(* REVIEW: make all callsites oo the htskipping thing, for other actions
           besides this.  Then only RegisterExpr remains, so inline it.
*) 
        IF HtSkipping 
        THEN (* NewExprRef becomes immediate garbage. *) 
        ELSE 
          FM3Exprs . RegisterExpr ( NewExprRef , Mergeable ) 
        END (*IF*)
      END HtExprRt 
  ...
  ; PROCEDURE HtScopeWPositionalDeclListRt
    ...
    = VAR LDeclList : FM3Globals . DeclListTyp 

(* TODO: Maybe inline this. *) 
    ; BEGIN (*HtScopeWPositionalDeclListRt*)
        LDeclList := FM3Decls . NewDeclList ( DeclCt )
      ; ScopeRef ^ . ScpDeclList := LDeclList
      END HtScopeWPositionalDeclListRt

  ; PROCEDURE HtScopeWPositionalDeclListLt ( ScopeNo : FM3Scopes . ScopeNoTyp )
    (* Just assert correct number of decrements occurred.*) 
  ...
  ; PROCEDURE HtExprOpnd1 ( )
    ...
    ; BEGIN 
      ...
      ; IF NOT HtSkipping 
        ...
        ; LParentExprRef := FM3Exprs . ExprStackTopObj 
        ; LParentExprRef ^ . ExpOpnd1 := LOpnd
        ; DefExprLt ( LParentExprRef ) 
        END (*IF*) 
      END HtExprOpnd1 
...
(*???*) ; HtPassTokenThru ( )
      ...
      =>  DeclValue ( "Default value of field" , MustBeConst := TRUE )
        ; HtPassTokenThru ( )
    ...
    =>  LPosition := GetBwdPos ( TokResult . TrRdBack )
      ...
      ; DeclValue ( "revelation" , MustBeConst := FALSE )
(* TODO:             ^This will give misleading error message. *) 
      ...
      =>  DeclType ( "CONST's type" )
        ; HtPassTokenThru ( )

      | Itk . ItkRecFieldDeclType 
      , Itk . ItkObjFieldDeclType 
      =>  DeclType ( "field's type" )
        ; HtPassTokenThru ( )

      | Itk . ItkVarDeclType 
      =>  DeclType ( "variable's type" )  
        ; HtPassTokenThru ( )

      | Itk . ItkVALUEFormalType
      , Itk . ItkROFormalType
      , Itk . ItkVARFormalType
      =>  DeclType ( "formal's type" ) 
        ; HtPassTokenThru ( )
      ...
      =>  DeclValue ( "method default" , MustBeConst := TRUE ) 
        ; HtPassTokenThru ( )

      | Itk . ItkMethodDeclSig
      =>  DeclType ( "method signature" )
(* TODO: Improve error messages emitted by DeclType. *) 
        ; HtPassTokenThru ( )

      | Itk . ItkMethodBodyAbsent
      =>  FM3Exprs . PushExprStack
            ( NEW ( FM3Exprs . ExprRefTyp
                  , ExpKind := Ekt . EkAbsent 
                  , ExpIsPresent := FALSE
      ...
      =>  IF FM3Exprs . ExprStackTopObj ^ . ExpKind # Ekt . EkObjType
          THEN <* ASSERT FALSE , "Method decl not in object type." *>
          END (*IF*)
        ; FM3Patch . DiscardOperands
            ( FM3Utils . TokenOpndCt ( TokResult . TrTok )
            , TokResult . TrRdBack
...
*)
      ...
      =>  LNewExprRef := FM3Exprs . NewExprRef ( )  
        ...
        ; FM3Utils . ContribToHashI ( LNewExprRef ^ . ExpHash , LOpcode ) 
        ; LNewExprRef ^ . ExpRepExprNo := FM3Exprs . RepExprNoDistinct 
        ; LNewExprRef ^ . ExpPosition := GetBwdPos ( TokResult . TrRdBack )
        ; LNewExprRef ^ . ExpIsLegalRecursive := TRUE
        ; LNewExprRef ^ . ExpKind := Ekt . EkNull
        ; LNewExprRef ^ . ExpUpKind := Ekt . EkNull
        ; LNewExprRef ^ . ExpIsUsable := FALSE 
          ...
          ; LNewExprRef (* An enum type. *) 
              := NEW ( FM3Exprs . ExprRefTyp
                     ...
                     , ExpArgList := FM3Exprs . ExprListTyp { NIL , LCt }
                       (* No need for a list, just use ElUnfilledCt to assign
                          values to the enum lits.
                       *) 
                     , ExpPosition := LPosition 
                     )
...
*)
      ...
      =>  LScopeNo := GetBwdScopeNo ( TokResult . TrRdBack ) 
        ...
        ; LNewExprRef
            := NEW ( FM3Exprs . ExprRefTyp
                   ...
                   , ExpOpcode := Stk . StkRwRECORD (* Needed? *)  
                   , ExpScopeRef1 := LScopeRef 
                   , ExpPosition := LPosition 
                   )
      ...
      =>  LPosition := GetBwdPos ( TokResult . TrRdBack )
        ...
        ; LExprRef := FM3Exprs . PopExprStack ( )
        ; IF FM3Exprs . ExprStackTopObj  ^ . ExpKind # Ekt . EkSignature
          THEN <* ASSERT FALSE , "Result type not in signature." *>
          END (*IF*)
      ...
      =>  LScopeNo := GetBwdScopeNo ( TokResult . TrRdBack )
        ; LCt := GetBwdInt ( TokResult . TrRdBack ) (* Formals. *)
        ; LCt2 := GetBwdScopeNo ( TokResult . TrRdBack ) (* Raise refs. *) 
        ; LPosition := GetBwdPos ( TokResult . TrRdBack )

        ; LExprRef := FM3Exprs . ExprStackTopObj 
        ; IF LExprRef ^ . ExpKind # Ekt . EkSignature
          THEN <* ASSERT FALSE , "ItkSignatureLt, not in signature." *>
          END (*IF*)
      ...
      (* Used in both OBJECT type and REF type.  We don't know which yet, so
         provide it unconditionally. It will be discarded later if it turns
         out to be REF. *)
...
(* 
          ...
          ; WITH WOverrideRef 
                    = LExprRef ^ . ExpDeclList . DlListRef 
                      ^ [ LExprRef ^ . ExpDeclList . DlUnfilledCt ]
            DO IF WOverrideRef ^ . ExpKind # Ekt . EkOverride 
              THEN <* ASSERT FALSE , "Expected override decl." *>
              END (*IF*)
            ; IF WOverrideRef ^ . ExpPosition # LPosition
...
*) 
      ...
      =>  IF NOT HtMaybePassTokenThru ( )
          ...
          ; BEGIN 
            ...
            ; LDeclCt := GetBwdInt ( TokResult . TrRdBack ) (* Fields. *) 
...
(* FIXME ^ or maybe sometimes EkConst. *)                     
      ...
      | Itk . ItkSubscriptRt 
      => HtCallOrSubscriptRt ( Ekt . EkSubscript ) 
      ...
      =>  LCt := GetBwdInt ( TokResult . TrRdBack ) (* Args count. *) 
        ...
        ; IF LCt > 0
          ...
          ; <* ASSERT LCt = LValueExpr ^ . ExpArgListNo *>
          END (*IF*)

      | Itk . ItkActualLt
      =>  LAtom := GetBwdAtom ( TokResult . TrRdBack ) (* Keyword. *) 
        ; LPosition := GetBwdPos ( TokResult . TrRdBack )
        ; FM3Exprs . ExprStackTopObj ^ . ExpIdAtom := LAtom 
...
(*
        ; IF LCt > 0
          ...
          ; <* ASSERT LValueExpr ^ . ExpArgListNo = 1 *> 
            DEC ( LValueExpr ^ . ExpArgListNo )
          ; LValueExpr ^ . ExpArgListRef ^ [ LValueExpr ^ . ExpArgListNo ]
              := LListElem
          END (*IF*)
*)
      ...
      =>  LCt := GetBwdInt ( TokResult . TrRdBack )
        ...
        ; LValueExpr := FM3Exprs . ExprStackTopObj (* Call or proc. *) 
        ; <* ASSERT LPosition = LValueExpr ^ . ExpPosition *>
          <* ASSERT LValueExpr ^ . ExpArgListNo = 0 *>
          FM3Exprs . FinishExprList ( LValueExpr ^ . ExpArgList ) 
        ; LValueExpr ^ . ExpArgPrefix := LPrefixExpr
        ; AdoptOutExprRef ( LValueExpr ) 
        ; PutBwdP2
            ( HtPass2RdBack , VAL ( LValueExpr ^ . ExpSelfExprNo , LONGINT ) ) 
...
; PROCEDURE AppendToScpDeclList ( DeclRef : FM3Decls . DeclRefTyp )

  = VAR LScopeRef : FM3Scopes . ScopeRefTyp

  ; BEGIN (*AppendToScpDeclList*)
      LScopeRef := FM3Scopes . ScopeDeclStackTopRef 
    ; IF NOT LScopeRef ^ . ScpKind IN FM3Scopes . ScopeKindSetPositional 
      THEN <* ASSERT FALSE , "Positional decl in wrong scope kind." *> 
      END (*IF*)
    ; IF LScopeRef ^ . ScpDeclListNo <= 0
      THEN <* ASSERT FALSE , "Too few positional decls." *>
      END (*IF*)
    ; WITH WDeclList = LScopeRef ^ . ScpDeclList 
      DO DEC ( WDeclList . DlUnfilledCt )
      ; WDeclList . DlListRef ^ [ WDeclList . DlUnfilledCt ] := DeclRef
      END (*WITH*) 
    END AppendToScpDeclList
...
; PROCEDURE ValidDeclIdR2L ( READONLY TokResult : TokResultTyp )
  ...
  ; PROCEDURE DidVisitDecl
    ...
    ; BEGIN (* DidVisitDecl *)
        IF DeclRefany # NIL
        THEN (* Some duplicate decls of the same ident were found earlier.  
                Dispense with them with error messages.
             *) 
      ...
      ; CASE DidDeclKind OF
        ...
        =>  AppendToScpDeclList ( LDeclRef )
          ...
          ; WITH WCt = LScopeRef ^ . ScpDeclList . DlUnfilledCt 
            DO
              DEC ( WCt )
            ; LValueExprRef ^ . ExpScalarConstVal := VAL ( WCt , LONGINT )  
            END (*WITH*) 
...
(* COMPLETEME *)
      ...
      ; IF FM3Scopes . ScopeDeclStackTopRef ^ . ScpKind
           IN FM3Scopes . ScopeKindSetOpen 
        THEN
          IF FM3Scopes . ScopeDeclStackTopRef
             # FM3Scopes . ScopeLookupStackTopRef
          THEN <* ASSERT FALSE , "Unequal self-referring scopes." *>
          END (*IF*) 
        ; IntSets . ForAllDo
            (* Refs inside the decl to decls in same open scope. *)
  ...
  ; BEGIN (*ValidDeclIdR2L*)
    ...
    ; IF VarArray_Int_Int . TouchedRange ( FM3Globals . SkipNoStack ) . Hi > 0
      THEN (* We are skipping output. *)
        RETURN FM3Globals . DeclNoNull 
      END (*IF*) 
...
; PROCEDURE SynthIsUsable1 ( Parent : FM3Exprs . ExprRefTyp )
...
; PROCEDURE SynthIsUsable2 ( Parent : FM3Exprs . ExprRefTyp )
...
; PROCEDURE SynthIsUsable12 ( Parent : FM3Exprs . ExprRefTyp )
...
; PROCEDURE CheckRecursiveRef ( RefDeclNo : FM3Globals . DeclNoTyp )
  ...
  ; BEGIN
    ...
    ; IF RefDeclNo
         >= LScopeMinDeclNo
            + IntSets . Card ( LLookupScopeRef ^ . ScpDeclIdSet ) 
      THEN RETURN
      END (*IF*)
    (* Ref is into the current top lookup scope.
       Would it be legal in a recursion?
    *) 
...
; PROCEDURE CheckOperation ( OpExpr : FM3Exprs . ExprRefTyp ) : BOOLEAN (* OK *) 

  = VAR LOK : BOOLEAN

  ; BEGIN
      IF OpExpr = NIL THEN RETURN TRUE END (*IF*) 
    ; LOK := TRUE
    ; IF NOT CheckParamKind
               ( OpExpr ^ . ExpOpnd1
               , OpExpr ^ . ExpOpcode 
               , "Left parameter"
               , OpExpr ^ . ExpBuiltinOpLtOpndKindsAllowed
               )
      THEN LOK := FALSE
      END (*IF*)
    ; IF OpExpr ^ . ExpBuiltinOpActualsCt > 1
         AND  NOT CheckParamKind
                    ( OpExpr ^ . ExpOpnd2 
                    , OpExpr ^ . ExpOpcode
                    , "Second parameter"
                    , OpExpr ^ . ExpBuiltinOpRtOpndKindsAllowed
                    )
      THEN LOK := FALSE
      END (*IF*)
    ; IF NOT CheckParamKind
      ...
      THEN LOK := FALSE
      END (*IF*)
    ; IF OpExpr ^ . ExpBuiltinOpActualsCt > 3
         AND NOT CheckParamKind
                   ( OpExpr ^ . ExpOpnd4 
                   , OpExpr ^ . ExpOpcode
                   , "Forth parameter"
...
; PROCEDURE AccumulateVarArg ( ExprRef : FM3Exprs . ExprRefTyp )
  (* PRE: ExprRef has not been pushed on expr stack. *) 

  = VAR LParentExprRef : FM3Exprs . ExprRefTyp
  
  ; BEGIN (*AccumulateVarArg*)
      LParentExprRef := FM3Exprs . ExprStackTopObj
    ; IF LParentExprRef = NIL THEN RETURN END (*IF*)
    ; IF NOT LParentExprRef ^ . ExpKind IN FM3Exprs . EkSetHasVarArgList
      THEN RETURN 
      END (*IF*)
    ; DEC ( LParentExprRef ^ . ExpArgListNo ) 
    ; LParentExprRef ^ . ExpArgListRef ^ [ LParentExprRef ^ . ExpArgListNo ]
        := ExprRef 
...
TRUE OR
          ...
          THEN (* Create an Expr node. *) 
            LExprIdentRef := FM3Exprs . NewExprRef ( ) 
...
(* TODO    ^ Compute this            *) 
              ...
              ; LExprRef := LExprRemoteRef
              (* NoArgs. *) 
              ; FM3Exprs . RegisterExpr ( LExprRef , Mergeable := FALSE ) 
              ; AdoptOutExprRef ( LExprRef )
              ELSE (* Emit an unusable token. *)
                PushExprIgnore ( LPosition , Ekt . EkInvalid )
...
TRUE OR
              AreInsideADecl ( ) 
          THEN (* Create an ExprIdNo node. *) 
            WITH WDotExpr = FM3Exprs . NewExprRef ( ) 
                 , WLtExpr = FM3Exprs . NewExprRef ( ) 
            DO 
              WDotExpr ^ . ExpOpnd1 := WLtExpr 
            ; WDotExpr ^ . ExpIdAtom := LAtomRt
            ; WDotExpr ^ . ExpPosition := LPosRt
            ; WDotExpr ^ . ExpKind := Ekt . EkDot
            ; WDotExpr ^ . ExpUpKind := Ekt . EkValue
(* FIXME ^ or maybe EkConst. *)                     
            (* NoArgs. *) 
            ; FM3Exprs . RegisterExpr ( WDotExpr , Mergeable := FALSE ) 
            ; AdoptOutExprRef ( WDotExpr )
...
(* TODO    ^ Compute this            *) 
              (* NoArgs. *) 
              ; FM3Exprs . RegisterExpr ( LExprRemoteRef , Mergeable := FALSE ) 
              ; AdoptOutExprRef ( LExprRemoteRef )
              ELSE 
                PutBwdP2 ( WOutRdBack , VAL ( LPosRt . Column , LONGINT ) ) 
              ; PutBwdP2 ( WOutRdBack , VAL ( LPosRt . Line , LONGINT ) ) 
              ; PutBwdP2 ( WOutRdBack , VAL ( LPosLt . Column , LONGINT ) ) 
              ...
              ; PutBwdP2
                  ( WOutRdBack
                  , VAL ( LIntfUnitRef ^ . UntSelfUnitNo , LONGINT )
                  ) 
              ; PutBwdP2
...
TRUE OR
            ...
            (* Turn it into separate QualId ref and dot Id *) 
              ...
              (* NoArgs. *) 
              ; FM3Exprs . RegisterExpr ( WDotExpr , Mergeable := FALSE ) 
              ; AdoptOutExprRef ( WDotExpr ) (* Which pushes. *) 
...
(* TODO    ^ Compute this            *) 
              (* NoArgs. *) 
              ; FM3Exprs . RegisterExpr ( WLtExpr , Mergeable := FALSE ) 
              ; AdoptOutExprRef ( WLtExpr ) (* Which pushes. *)
              END (*WITH*)
