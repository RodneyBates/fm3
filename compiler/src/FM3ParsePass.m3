...
; PROCEDURE CheckQualUnreserved ( READONLY StkIdAttribute : tParsAttribute )
  : BOOLEAN (* It's OK. *) 
...
; PROCEDURE QualIdentL2R
  ...
  = BEGIN (*QualIdentL2R*)
      ...
      DO
        IF CheckQualUnreserved ( StkLtIdAttribute )
           AND CheckQualUnreserved ( StkRtIdAttribute )
        THEN 
          WITH WIdentRefSet = FM3Scopes . ScopeStackTopRef ^ . ScpRefIdSet
          DO WIdentRefSet
               := IntSets . Include
...
; PROCEDURE QualIdentL2R
  ...
  ; VAR LPositionLt , LPositionRt : FM3Base . tPosition
  ; VAR LDeclNo : FM3Base . DeclNoTyp
  ...
  ; BEGIN (*QualIdentL2R*)
      WITH WScope = FM3Scopes . ScopeStackTopRef ^  
           , WppRdBack
             = FM3Units . UnitStackTopRef ^ . UntParsePassRdBack 
      DO
        LAtomLt := GetBwdAtom ( WppRdBack )
      ; LAtomRt := GetBwdAtom ( WppRdBack )
      ; LPositionLt := GetBwdPos ( WppRdBack )
      ; LPositionRt := GetBwdPos ( WppRdBack )
